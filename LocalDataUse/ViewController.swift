...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //데이터를 가져와서 출력
        //email이라는 key에 데이터가 없으면 nil이 되고 예외가 발생
        //nil이 아닌 경우만 수행
        ...
        //값이 nil이면 false를 리턴하고 그렇지 않으면 true를 리턴
    ...
    @IBAction func read(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "파일읽기", style: .default))
    ...
    @IBAction func remove(_ sender: Any) {
        ...
        //파일 삭제
       // try! fileMgr.removeItem(at: URL(string: filePath)!)
        try! fileMgr.removeItem(atPath: filePath)
    }
    
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        let bundleViewController = self.storyboard?.instantiateViewController(withIdentifier: "BundleViewController") as! BundleViewController
